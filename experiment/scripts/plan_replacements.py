...
def replacement_matrix(valid: int, required: int, maximum: int, *, attempted: int | None = None) -> dict[str, object]:
    # An absent result has an unknown cause and must not silently become a
    # replaceable measurement failure. Completed invalid pairs are classified.
    completed = required if attempted is None else min(attempted, required)
    missing = max(0, completed - valid)
...
def main() -> None:
    ...
    matrix = replacement_matrix(valid, args.required, args.maximum, attempted=len(pairs))
